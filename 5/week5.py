...
ex1 ="/".join([position,file['ex1']])

df = pd.read_csv(ex1)
df
pd.read_table(ex1, sep=',')

ex2 = "/".join([position,file['ex2']])
pd.read_csv(ex2)
pd.read_csv(ex2, header=None)
pd.read_csv(ex2, names=['a', 'b', 'c', 'd', 'message'])


names = ['a', 'b', 'c', 'd', 'message']
pd.read_csv(ex2, names=names, index_col='message')

mindex = "/".join([position,file['mindex']])
pd.read_csv(mindex)
parsed = pd.read_csv(mindex, index_col=['key1', 'key2'])
parsed

ex3 = "/".join([position,file['ex3.txt']])
list(open(ex3))
result = pd.read_table(ex3, sep='\s+')
result

ex4 = "/".join([position,file['ex4']])
pd.read_csv(ex4, skiprows=[0, 2, 3])

ex5 = "/".join([position,file['ex5']])
result = pd.read_csv(ex5)
result
pd.isnull(result)

result = pd.read_csv(ex5, na_values=['NULL'])
result

sentinels = {'message': ['foo', 'NA'], 'something': ['two']}
pd.read_csv(ex5, na_values=sentinels)

sentinels = {'message': ['foo','foo'], 'something': ['one',"three"]}


#逐行读取文本文件
ex6 =  "/".join([position,file['ex6']])
